            ...
            <ul class="cart-item-list">
              <li v-for="(item,index) in cartList" :key="index">
                ...
                <div class="cart-tab-4">
                  <div class="item-price-total">{{parseInt(item.salePrice)*item.productNum | currency("￥")}}</div>
                </div>
  ...
  export default{
    ...
    computed: {
      ...
      totalPrice(){
        ...
        this.cartList.forEach((item)=>{
          if(item.checked == 1){
            total += parseFloat(item.salePrice)*parseFloat(item.productNum)
          }
        })
        return total;
      }
      ...
      delCart () {
        axios.post("users/cartDel",{
          ...
          if(res.status == '0'){
            ...
            // this.init();
            //少调用一次请求
            this.cartList = this.cartList.filter((value)=>value.productId != this.delItem.productId);
